...
DeleteField::DeleteField(char *line) : WriteIdrTestCase(line) {
}
...
const char *DeleteField::getName() {
    return "DeleteField";
}
...
const char *DeleteField::getDescription() {
    return "Delete an Inventory Data Field.";
}
...
const char *DeleteField::getPrecondition() {
...
SaErrorT DeleteField::getExpectedReturn() {
    return SA_OK;
}
 ...
 * Run the test.
 *
 * For each area, add a field and then delete it.
 *****************************************************************************/

HpiTestStatus DeleteField::runWriteIdrTest(SaHpiSessionIdT sessionId,
...
/*****************************************************************************
 * Add a field to an area and then try to delete it.
 *****************************************************************************/

HpiTestStatus DeleteField::deleteField(SaHpiSessionIdT sessionId,
    ...
    if (error == SA_ERR_HPI_INVALID_DATA) {
        ...
        if (error != SA_OK) {
            status.assertFailure(TRACE, IDR_FIELD_DELETE, SA_OK, error);
        } else {
            status.assertPass();
        }
    }
