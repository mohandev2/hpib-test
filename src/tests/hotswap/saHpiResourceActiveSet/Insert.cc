...
const char *Insert::getDescription() {
    return "Place a resource into the INSERTION PENDING state and then\n"
        ...
        if (error != SA_OK) {
            ...
            if (status.isOkay()) {
                if (hsState == SAHPI_HS_STATE_INACTIVE) {
                    ...
                    // restore resource to original inactive state
...
/*****************************************************************************
 * Test a resource by doing a software insertion and then canceling the
 * insertion policy.  At this point we can continue the insertion by setting
 * the resource to active. 
 *****************************************************************************/
