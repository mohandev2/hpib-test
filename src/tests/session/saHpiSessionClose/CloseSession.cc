...
CloseSession::CloseSession(char *line) : HpiGenTestCase(line) {
}
...
const char *CloseSession::getDescription() {
    return "Close an opened session.";
}
...
HpiTestStatus CloseSession::run() {
    ...
    if (error != SA_OK) {
        ...
        if (error == SA_OK) {
            ...
            status.assertFailure(TRACE, SESSION_CLOSE, SA_OK, error);
        }
    }
