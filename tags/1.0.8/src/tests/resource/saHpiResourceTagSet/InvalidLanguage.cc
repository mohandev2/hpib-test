...
const char *InvalidLanguage::getDescription() {
    return "The <i>Language</i> is not one of the enumerated values "
           "for that type when the <i>DataType</i> is SAHPI_TL_TYPE_UNICODE "
...
HpiTestStatus InvalidLanguage::runTagTest(SaHpiSessionIdT sessionId,
    ...
    for (int i = 0; i < 2; i++) {
        TextBufferHelper::fillByDataType(&tag, dataType[i]);
        tag.Language = (SaHpiLanguageT) invalidLanguage;
