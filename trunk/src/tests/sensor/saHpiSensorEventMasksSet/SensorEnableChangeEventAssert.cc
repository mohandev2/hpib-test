...
SensorEnableChangeEventAssert::SensorEnableChangeEventAssert(char *line)
...
const char *SensorEnableChangeEventAssert::getName() {
    return "SensorEnableChangeEventAssert";
}
...
const char *SensorEnableChangeEventAssert::getDescription() {
    return "If a sensor's <i>AssertEventMask</i> changes as a\n"
           "result of a call to saHpiSensorEventMasksSet(), a Sensor\n"
           "Enable Change Event will be generated.";
}
...
const char *SensorEnableChangeEventAssert::getPrecondition() {
    return "To perform this test, one or more sensors must be found\n"
           "whose assert event masks can be changed.";
}
...
HpiTestStatus SensorEnableChangeEventAssert::runSessionTest(SaHpiSessionIdT sessionId) {
...
HpiTestStatus SensorEnableChangeEventAssert::testResource(SaHpiSessionIdT sessionId,
...
bool SensorEnableChangeEventAssert::canTestRdr(SaHpiRdrT *rdr) {
...
HpiTestStatus SensorEnableChangeEventAssert::testSensor(SaHpiSessionIdT sessionId,
                                                        SaHpiResourceIdT resourceId,
    ...
    if (error != SA_OK) {
        ...
        if (assertEventMask == 0) {
            sensorData[sensorCount].nextAction = REMOVE_EVENT_STATES;
            status.add(TRACE, changeEventMasks(sessionId, resourceId,
                                               sensorRec->Num,
                                               SAHPI_SENS_ADD_EVENTS_TO_MASKS));
        } else {
            sensorData[sensorCount].nextAction = ADD_EVENT_STATES;
            status.add(TRACE, changeEventMasks(sessionId, resourceId,
                                               sensorRec->Num,
...
HpiTestStatus SensorEnableChangeEventAssert::pollEvents(SaHpiSessionIdT sessionId,
                                                        long timeout) {
    HpiTestStatus status;
...
HpiTestStatus SensorEnableChangeEventAssert::handleEvent(SaHpiSessionIdT sessionId,
                                                         SaHpiEventT *event) {
    HpiTestStatus status;
    ...
    for (int i = 0; i < sensorCount; i++) {
        if ((sensorData[i].resourceId == event->Source) && 
            ...
            (sensorData[i].foundEventCount < 2)) {

            sensorData[i].foundEventCount++;

            if (sensorData[i].nextAction == ADD_EVENT_STATES) {
                status.add(TRACE, changeEventMasks(sessionId,
                                                   sensorData[i].resourceId,
                                                   sensorData[i].sensorNum,
                                                   SAHPI_SENS_ADD_EVENTS_TO_MASKS));
            } else if (sensorData[i].nextAction == REMOVE_EVENT_STATES) {
                status.add(TRACE, changeEventMasks(sessionId,
                                                   sensorData[i].resourceId,
                                                   sensorData[i].sensorNum,
                                                   SAHPI_SENS_REMOVE_EVENTS_FROM_MASKS));
 ...
 * entire timeout period.  In this test, there are two events per sensor.
 *****************************************************************************/

bool SensorEnableChangeEventAssert::isEventProcessingDone() {
    return (foundEventCount == sensorCount * 2);
}

/*****************************************************************************
 * Check all of the sensors that were tested.  We should have recieved a
 * two sensor enable change events for each.  If not, then record a failure 
 * for each of those sensors.
 *****************************************************************************/

HpiTestStatus SensorEnableChangeEventAssert::verifyEvents() {
    HpiTestStatus status;
...
HpiTestStatus SensorEnableChangeEventAssert::restoreSensors(SaHpiSessionIdT sessionId) {
    HpiTestStatus status;
...
HpiTestStatus SensorEnableChangeEventAssert::changeEventMasks(SaHpiSessionIdT sessionId,
                                                              SaHpiResourceIdT resourceId,
...
HpiTestStatus SensorEnableChangeEventAssert::resetEventMasks(SaHpiSessionIdT sessionId,
                                                             SaHpiResourceIdT resourceId,
                                                             SaHpiSensorNumT sensorNum,
