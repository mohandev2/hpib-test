...
void InvalidRequest::addWatchdog(SaHpiSessionIdT sessionId,
				 SaHpiResourceIdT resourceId,
				 SaHpiWatchdogNumT watchdogNum,
				 SaHpiWatchdogT &origWatchdog) {

  /***
   * The PretimerInterrupt is an optional feature in some hardware platforms like e.g.
   * ATCA or in common IPMI based architectures.
   * So it must be verified if the Pretimer can be used by setting it one time (see also 
   * "21.4.1 Pre-timeout Interrupt Support Detection", IPMI spec. v1.5, Rev. 1.1)
   ***/
